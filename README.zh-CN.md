...
默认情况下，只有 `SKILL.md` 应该作为 Skill 主入口被加载。`references/` 下的参考资料和 `examples/` 下的示例，应该只在当前任务确实需要时再读取。

推荐使用方式：

- 非平凡工程任务中按需使用；
- 不建议在一次性闲聊、纯只读问答中默认启用；
- 团队希望统一 Agent 工作方式时，优先使用项目 / 工作区级安装；
- 个人希望在大多数编码项目中保持同一套工作纪律时，可以使用全局安装。

建议触发策略：

- 日常闲聊、只读问答：默认不触发；
- 功能开发、Bug 修复、重构、依赖升级、API 改动、配置变更、高风险修改：建议触发；
- 需要检查分支、Git 状态、验证证据、风险和交接状态时：建议触发。
